...
table(TABLA$Tipo)
```
Podemos afirmar que no es equilibrado, ya que no hay el mismo número de observaciones para cada factor. Esta característica es difícil de cumple en nuestro caso puesto que no siempre es posible tener el mismo número de pacientes con cáncer de próstata, así como pruebas bioquímicas de sus niveles de PSAt.
...
A la vista del primer podemos intuir que tenemos cierta condición de homocedasticidad, ya que las varianzas no difieren del todo. 
